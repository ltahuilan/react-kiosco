...
export default function Categoria( {categoria} ) {
    ...
    return (
        ...
        <button
            className={`${ categoriaSeleccionada() } w-full flex gap-6 items-center p-3 dark:text-slate-50 font-bold border-b border-slate-400 hover:bg-amber-400 dark:hover:bg-amber-500 transition-colors truncate cursor-pointer`}
            onClick={ () => handleClickCategoria(id) } //sintaxis de arrow function para que se ejecute hastaque ocurra el evento
        >
            <img
                src={ `/img/icono_${icono}.svg` }
